...
node lib/selftest.mjs                                    # offline, no key
node run.mjs <suite.json> --print-prompt=live            # eyeball the payload, free
node run.mjs <suite.json> --dry-run --out=/tmp/dry       # whole pipeline, no charge
export ANTHROPIC_API_KEY=...                             # env only, never a file
node run.mjs <suite.json>                                # spends calls
```
...
```js
const myCall = {
  templateDir: 'mycall',          // templates/mycall/base-v0.4.md + user-v0.4.md
...
`reporter` entry is already declared as a prose-output example (no tool) for when
E-CONT screening needs the report leg; wire its templates when that lands.
