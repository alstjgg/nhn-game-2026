...
## As executed (08-07, restored to main 08-08)

The heaviest §5.7 traffic of the wave: **four** correct stops, each a defect in
this document rather than in the tree, and each folded back in above as a
reissue.

1. Edit 21 assumed `run-loop.spec.ts` carried a harness import list. It carries
   none — split into 21a/21b with an explicit new import.
2. Edit 22a's `frame` import collided with `reports.spec.ts`'s own local
   `frame` at `:78` — narrowed to `mineFirst, slotBlock`.
3. The first issue claimed `fonts-css.test.ts (f)` needed no edit. Wrong, and
   the reason is the interesting one: `(f)` reads `dirAtUnit('u10', …)`, a
   FROZEN git snapshot of u10's own merge, where `win-block-store.css` always
   exists — so trimming the live baseline row makes the sheet read as a
   spurious u10 addition. `index-order.test.ts` was missing from Scope entirely
   for the same reason (its `U1_WINDOW_SHEETS` derives from the live list).
4. The Done-when grep exposed a live `w-store` in `selectors.ts`'s `WINDOWS` /
   `WINDOW_IDS` and in `debug-pane.spec.ts` — items 25/26.

**Why this file was out of date on main.** The reissues were written into the
executor's worktree copy; the docs lane committed an earlier issue, so main
carried a version claiming `(f)` needed no edit while the landed code has the
carve-out. Found during worktree teardown (08-08) by byte-comparing each
worktree's untracked PRD against main before deleting it, and restored here.
