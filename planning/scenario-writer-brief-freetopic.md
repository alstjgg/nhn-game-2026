# 집필 브리프 — 자유주제 (그래프 선행 방식, v3.1-free)
...
무슨 재난인지, 어느 도시인지, 그날 누가 당직이었는지, 무엇이 감춰져 있는지 —
전부 당신이 짓는다. 정해진 것은 아래 물리뿐이고, 이것만 지키면 어떤 무대든
열려 있다.

- **재난은 신고 한 통에서 시작한다.** 요원은 그 신고를 받고 파견된다. 그
  신고가 무엇을 말하는지, 말하지 않는지가 첫 게이트의 재료다.
- **사람이 많고, 빨리 빠져나갈 수 없는 자리여야 한다.** 그리고 시계가 거기
  닿아야 한다. 경기장, 여객선, 출근 시간 승강장, 계단이 하나뿐인 탑, 눈에
  갇힌 국도. 그래야 기록의 사망자가 **세 자리**가 되고, 그래야 채굴 한 번의
  값이 목숨 하나가 아니라 수십이 된다. 한 자릿수 사망은 재난이 아니라
  퍼즐이다.
- **가장 좋은 결말의 사망자는 0명이다.** 반드시 도달 가능해야 한다. 잘 된
  시행에도 값은 남지만, 그 값은 **목숨이 아닌 것으로** 치른다 — 누군가 입건되고,
  건물이 폐쇄되고, 이름 옆에 기록이 남는다.
- **재난 자체는 막지 못한다.** 지붕은 그대로 내려앉고 배는 그대로 기운다.
  요원이 바꾸는 것은 그때 거기 누가 서 있었는가다.
...
- **한 시행은 두세 시간이고, 자정을 넘지 않는다.** 이 제한은 **결과 꼬리와
  종단 시각까지 포함한 모든 시각**에 걸린다(§4-14). 시계는 하루 안에서만
  돌기 때문에, 00:12 같은 시각이 하나라도 있으면 그 시행은 만들어지는 순간
  이미 끝나 있다. 낮에 벌어지는 재난이어도 된다 — 다만 마지막 종단이 그날
  안에 닫혀야 한다.

- **터널 화재는 쓸 수 없다.** 다른 과제가 이미 가져갔다. 그 밖에는 제한이
  없다.

**로그라인이 답이어야 하는 질문.** 이 재난은 다른 재난과 무엇이 다른가, 요원이
여기서 무엇에 걸려 넘어지는가, 감춰진 것이 왜 하필 감춰졌는가. 세 문장 안에
그 답이 서 있으면 나머지는 따라온다.
...
4. **열쇠는 문 뒤에 있다 — 그리고 창 안에서 조립되지도 않는다.** 어떤 간선의
   requires에 든 앎은 (가) 시작에서 그 게이트까지 가는 **어떤 경로 위의 노드도
   yield하지 않고**, (나) **§1이 이미 준 것도 아니고**, (다) **그 게이트에서
   요원이 보는 타임라인 행들로 조립되지도 않는다.**

   (다)가 늦게 붙은 항목이고, 실측에서 게이트 하나를 통째로 죽였다. 열쇠가
   「지하와 2층은 급기가 한 계통이다」였는데, 요원의 창에 이미 두 행이 있었다 —
   신고자는 무대 쪽으로 갈수록 맵다고 하고, 시설 쪽은 1층 후드라고 한다.
   요원은 계통도가 필요하지 않았다. 두 행을 나란히 놓기만 하면 원인 설명이
   현장을 설명하지 못한다는 것이 서고, 그것이 곧 비기본 스탠스의 전제였다.
   세 팔이 전부 같았고 기본 stance는 한 번도 나오지 않았다.

   **그러므로 검사는 「어느 노드가 이 앎을 내놓는가」가 아니라 「요원이 자기
   창만으로 이 전제를 세울 수 있는가」다.** 게이트마다 그 창에 들어가는 행을
   전부 적어 놓고, 비기본 라벨의 전제를 하나씩 대 보라. 창 안에서 서면 그
   열쇠는 열쇠가 아니다.

   타임라인을 맨 마지막에 쓰기 때문에 이 실수가 특히 쉽다. 행은 경로가
   필요로 해서 놓이고, 놓인 뒤에 그 행이 무엇을 먼저 말해 버리는지는
   아무도 다시 보지 않는다.
...
13. **첫 시행의 꼬리는 앎을 둘 이상 내놓되, 전부 내놓지는 않는다.** 하나만
    ...
    둘째 시행이 형식이 된다. 가장 깊은 앎 하나 이상은 둘째 시행 이후의
    꼬리에서만 나오게 하라.
    (§4-5가 「시행마다 게이트 하나씩」으로 읽히는 자리가 있는데, 이 항목과
    맞지 않는다 — 첫 꼬리가 앎 둘을 내놓으면 둘째 시행은 게이트 둘을 지난다.
    강제되는 것은 §4-7(셋째 시행 안에 성공이 열린다)이고, 게이트가 몇 번째
    시행에 떨어지느냐는 그 아래에서 자유롭다.)
14. **모든 시각이 자정 전이다.** 게이트, 종단, 타임라인 행, 그리고 **결과 꼬리
    안에서 서술되는 모든 시각**까지 전부다. 마지막 종단이 23시대 안에 닫히도록
    시작 시각을 잡아라.

15. **게이트는 무엇을 믿을 것인가, 그래서 다음에 무엇을 물을 것인가를 묻는다.
    무엇을 명령할 것인가를 묻지 않는다.** 이것이 이 브리프에서 가장 비싸게
    확인된 규칙이다.

    두 팩 열두 게이트를 재고 나서 남은 것은 이 하나다. 살아남은 게이트의
    비기본 행동은 전부 **묻거나 조회하는 일**이었다 — 인원을 다시 세게 한다 ·
    편성을 따로 확인한다 · 벽을 따라 찾게 한다. 죽은 게이트의 비기본 행동은
    전부 **명령**이었다 — 비상문을 연다 · 영하로 사람을 내보낸다 · 책임자를
    회선에서 내린다 · 이백 명을 옮긴다.

    까닭은 요원의 처지에 있다. §1이 요원의 권한을 청취 · 조회 · 요청으로
    적어 두었고 집행권이 없다고 못 박았다. 명령은 그 권한 밖이고, 게다가
    명령은 값이 즉시 읽히는데 이득은 열쇠가 있어야 읽힌다 — 그래서 요원은
    싼 쪽으로 물러나거나, 기본까지 명령이면 열쇠를 무시하고 명령들끼리
    비교해 버린다.

    **그러므로 게이트를 세울 자리를 고를 때 이것부터 물어라 — 이 자리에서
    요원이 갈리는 것은 무엇을 믿느냐인가, 무엇을 시키느냐인가.** 뒤쪽이면
    그 자리는 게이트가 아니다. 장면이거나, 다른 게이트의 결과다.

    딸린 규칙 둘. **선택지는 순위를 매길 수 있는 것이 아니라 서로 견줄 수
    없는 것이어야 한다** — 견줄 수 없게 만드는 것은 값이다(§5-14). 그리고
    **비기본 행동이 그 자체로 옳아 보이면 안 된다** — 갇힌 사람이 있는데
    비상문을 여는 것은 아무것도 믿지 않아도 옳으므로 어떤 열쇠도 걸리지
    않는다.
---

## 5. 스탠스 — 요원이 보는 것은 라벨뿐이다

> **이 절은 전부 실측에서 왔다.** 프로브로 확인된 것만 여기 있다. 반면 §4의
> 항목 가운데 3·5·6·7·13은 아직 한 번도 측정되지 않았다 — 한 자리에 앉아
> 네 시행을 끝까지 해 본 적이 없기 때문이다. 그 다섯은 설계 의도이고, 이
> 절의 열넷은 법이다. 둘이 부딪히면 이 절이 이긴다.
...
11. **비기본 라벨은 전제를 말하되, 그 전제를 여는 진실의 내용을 부르지 마라.**
    이것이 실측에서 가장 비싸게 배운 것이다. 진실이 「북측 문은 사슬로 묶여 있고
    열쇠는 당직자 주머니에 있다」인데 라벨이 `사슬 때문이라고 보고`라고 적으면,
    요원은 스탠스 세트를 읽는 것만으로 그 진실을 알게 된다. 넘겨줄 것이 남지
    않으므로 열쇠가 열쇠가 아니게 되고, 빈 인수인계가 그 문을 연다.
    전제는 열쇠가 **licence하는 해석**이지 열쇠가 **말하는 내용**이 아니다.
    (실측: 사물을 이름으로 부른 게이트는 무개입 baseline이 70%·10%까지 떨어졌고,
    해석만 말한 게이트는 100%였다.)
12. **전제는 세계에 관한 것이어야 한다. 방법에 관한 전제는 싸다.**
    `그가 감추고 있다`는 증거가 있어야 서지만 `그에게 물어서는 안 나온다`는
    기질만으로도 선다. 후자로 고쳐 쓴 스탠스는 미끼 문장 하나에 10/10으로
    열렸다 — 미끼가 열쇠가 된 것이다.
13. **갈래를 만드는 것은 스탠스 수가 아니라 전제의 값이다.** 넷을 놓아도 하나가
    공짜면 90%가 그리로 몰린다. 값이 비슷해야 흩어진다 — 한 게이트에서 값이
    새던 스탠스를 고치자, 손대지 않은 다른 스탠스가 0에서 다섯으로 살아났다.
14. **모든 스탠스가 무언가를 치러야 한다. 준비에 그치는 스탠스는 값이 0이고,
    값이 0인 선택지는 언제나 이긴다.**
    「보낸다」 · 「묻는다」 · 「자리를 잡는다」는 되돌릴 수 없는 일이 아니라서
    틀릴 수가 없다. 그 옆에 무언가를 닫는 기본 stance를 놓으면, 세계가 무슨
    경고를 해도 준비 쪽이 이긴다 — 경고가 걸릴 자리가 없기 때문이다.
    스탠스는 **하려는 일에 스스로를 건다**: 설비 쪽으로 보내는 것이 아니라
    지금 돌리게 하고, 까닭을 묻는 것이 아니라 지금 열게 한다.
    (실측: 비기본 셋이 전부 준비였던 게이트는 라벨을 세 번 고치고 세계에 물리를
    넣어도 무개입 baseline이 0%였다. 네 선택지가 전부 값을 치르는 게이트는
    100%였다.)
---
