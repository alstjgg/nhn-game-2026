...
15. **게이트는 비기본 행동이 그 자체로 옳아 보이지 않는 자리에만 선다.**
    선택지들이 같은 믿음 위에서 속도나 효율로만 갈리면 그것은 판단이 아니라
    최적화이고, 최적화에는 정답이 있다. 어떤 열쇠도 정답인 행동을 막지 못한다.
    G1의 `인원을 다시 세게 한다`와 G2의 `벽을 따라 찾게 한다`는 무언가를
    믿어야만 말이 되는 행동이라 열쇠가 걸렸다. `갇힌 사람이 있는데 비상문을
    연다`는 아무것도 믿지 않아도 옳으므로 걸리지 않는다.
    **선택지는 순위를 매길 수 있는 것이 아니라 서로 견줄 수 없는 것이어야 한다.**
    견줄 수 없게 만드는 것은 값이다 — §5-14를 함께 읽어라. 값이 다른 것끼리
    맞서야 순위가 생기지 않는다.
---
