...
## EXC 2026-08-09 — 선언된 창으로 다시 잰 전구간정상 세 갈림길

읽기: [`runs/EXC-20260809-declared-window-read.md`](runs/EXC-20260809-declared-window-read.md)
· 수트 `suites/EXC-G{1,2,3}-declared-window.json` · 70콜 (7팔 × N=10)

갈림길 카드가 `excerpt`로 자기 창을 행 id로 선언하는 기능이 들어온 뒤의 첫 측정이다
([설계 기록](../research/gate-excerpt-design.md)). G2는 `a` 9/10 · k2 → `b` 9/10으로
PR #214가 열어 둔 칸을 닫았고 — 그 4/6은 수동 프로토콜이 자기 픽스처를 재고 있었다는
쪽을 지지한다 — G3은 `a` 9/10에 k3 → `b` 7/10, k4 → `c` 9/10으로 두 열쇠가 다른 문을
열었다. G1은 drop됐다.

### A23 · 선언된 창은 정보가 아니라 최근성으로 고른다

G1의 창을 t1·t2·t6으로 선언했더니 baseline이 `b`×10으로 나왔다. 사전등록은 `a`였고,
PR #214가 fallback 창에서 손으로 잰 값도 `a`였다. 열 콜 전부가 t1·t2의 불과 연기를
「관제 화면 '전 구간 정상'과 직접 어긋난다」의 근거로 들었다 — 그 대립이 바로 이
갈림길이 묻는 것이고, 창이 답을 미리 건넨 것이다.

fallback이 `a`를 내던 이유가 여기서 드러난다. 6줄 상한은 20:47·20:49를 **오래되었다는
이유로** 잘라내고 있었다. 상한이 하던 일은 정보 선별이 아니라 최근성이었다.

*Operating rule.* 창을 선언할 때는 「가장 정보가 많은 행」이 아니라 **갈림길 직전의
행부터** 잡는다. 오래된 행을 되살리려면 그 행이 기본값과 어긋나지 않는지 따로
확인한다 — 상한이 가려 주던 것을 선언은 가려 주지 않는다. W5(자기 열쇠 금지)는 이것을
잡지 못한다: t1·t2는 G1의 `key_examples`가 채굴한 시각이 아니므로 린트는 침묵했고,
그래도 창은 답을 넘겼다. 규칙 6은 여전히 사람이 읽어야 하는 절반이 있다.

### A24 · 갈림길의 전제가 타임라인 행에 없으면 창은 그것을 실을 수 없다

G1의 질문은 화면이 '전 구간 정상'이라고 말한다는 사실 위에 서는데, 그 사실은 카드의
`scene` 산문에만 있고 타임라인 행에는 없다. `scene`은 모델에 가지 않는다(`GateView`는
질문·스탠스·창·기질뿐). fallback에서는 Call 2 산문이 그 전제를 우연히 실어 주고 있었다.

*Operating rule.* 창을 선언하기 전에, 그 갈림길의 질문이 전제하는 사실이 **행으로
존재하는지** 확인한다. 없으면 셋 중 하나다 — 행을 세우거나, 그 갈림길만 창을 선언하지
않고 fallback에 두거나(필드는 갈림길마다 선택이다), 질문을 전제가 덜 필요한 형태로
고치거나. 선언은 우연히 실려 오던 것을 드러내지, 대신 실어 주지 않는다.

### A25 · 갈림길의 창은 언제나 직전 한 비트다 — 실측

`bindRun`에 캡처 transport를 물려 무개입 런의 Call 1 요청을 그대로 떴다. 비트마다 네 줄이
기록되고(스크립트 행 · 증상 · Call 2 서술 · NPC 대사) `TIMELINE_CAP_LINES = 6`이 비트
단위로 자르므로, **갈림길에 들어가는 저작된 문장은 언제나 하나**다. 전구간정상 세 갈림길
모두 그랬다.

같은 캡처가 `(변화 없음)`의 출처도 밝혔다. 픽스처 Call 2가 `SCENE_SYMPTOMS`를
`timeline_entries`에 복사하므로(`src/transport/fixture.ts`) 그 문자열은 엔진이 실제로
보내는 것이다 — 수동 프로토콜의 실수가 아니었다. 실제 런에서는 그 자리에 Call 2 산문이 온다.

*Operating rule.* fallback 창을 가정하지 말고 뜬다. 캡처는 LLM 호출 없이 픽스처
provider로 되므로 공짜다. 「갈림길 앞 여섯 줄」로 수트를 쓰면 실제보다 다섯 줄 많은 창을
재게 된다.

### A26 · 기본값이 서지 않는 갈림길은 창으로도 라벨로도 구제되지 않는다

전구간정상 G1에서 창을 세 번(3행 → 2행 → 1행), 라벨을 한 번(전제를 「달리 볼 이유가
없으므로」에서 절차로) 바꿔 40콜을 썼다. 최빈 stance는 네 번 다 `b`였고 `a`는 최대 2/10이었다.

원인은 질문이다. 「화면의 '전 구간 정상'을 근거로 삼을 것인가」는 요원에게 화면의 신빙성을
평가하라는 주문이고, 21:04 이전의 모든 행은 사고를 서술하므로 평가의 재료가 된다. 어긋나지
않는 창은 빈 창뿐이다.

*Operating rule.* 기본값이 서지 않을 때 창과 라벨을 각각 한 번씩 재고도 최빈이 안 바뀌면,
레버를 더 당기지 말고 **갈림길이 포화됐다(A9·A10)고 기록한다.** 다음은 질문과 stance set의
재설계이고 그것은 별도의 작업이다. 특히 이 실패는 무개입 기준 점수의 전제를 건드리므로 —
그 갈림길의 기본 버킷이 flag를 세우면 첫 런의 사다리 값이 달라진다 — 점수표를 함께 다시
본다.

### A27 · A26을 좁힌다 — 창 레버는 「정상화하는 줄」을 아직 안 써 봤다

A26은 창과 라벨을 다 재고 「갈림길이 포화됐다」고 적었다. 창 쪽 결론은 **사고를 서술하는
행만** 넣고 빼 본 것이다. 뒤이어 픽스처가 얹힌 창(`t6` + `(변화 없음)` + 픽스처 두 줄)을
같은 모델·같은 라벨로 재니 `a`가 2/10에서 6/10으로 올라 최빈이 뒤집혔다. 같은 창을 큰
모델(Opus 5)로 재면 `b` 10/10이므로 모델 차이는 아니다.

표본이 이유를 말한다 — 「달리 볼 이유가 없다. 관제 화면의 '전 구간 정상'과 상충하는 정보가
없고」. 기본 stance의 라벨이 「달리 볼 이유가 없으므로」로 시작하면, 그 전제를 참으로 만드는
것은 **반증의 부재를 말하는 줄**이다.

*Operating rule.* 기본값이 서지 않을 때 창에서 사고 행을 빼는 것만으로 결론짓지 않는다.
`반증의 부재`를 말하는 줄을 하나 넣어 보고 나서 포화를 선언한다. 그리고 이것은 A9·A10의
포화 판정에도 붙는다 — 「어떤 창으로도 안 된다」는 정상화 줄을 시험한 뒤에만 할 수 있는 말이다.

부수적으로, 이것이 PR #214의 G1 `a`를 설명한다. 그 프로토콜의 페이로드에는 픽스처 세 줄이
들어 있었고, 배포에서 그 자리에 오는 것은 Call 2가 그 런에서 쓴 산문이다 — 즉 그 갈림길의
기본값은 Call 2가 마침 무엇을 썼는지에 달려 있었다. 선언된 창이 없애는 것이 정확히 그
의존이다.

### A28 · 기본 stance가 「달리 볼 이유가 없으므로」로 시작하면, 창에 그 부재를 말하는 줄이 있어야 한다

전구간정상 G1을 일곱 번 쟀다. 창을 세 번 줄이고, 라벨의 전제를 절차로 옮기고, 모델을
올려도 최빈은 `b`였다. `a`가 선 것은 두 경우뿐이고 둘 다 창에 **반증의 부재를 말하는 줄**이
있을 때였다 — 픽스처의 `(변화 없음)`(a 6/10), 그리고 그 역할을 대신하도록 저작한 21:03 행
(a 7/10). 후자에서 k1 주입은 `b` 10/10으로 그대로 열렸다.

*Operating rule.* 기본 stance의 라벨이 근거의 부재를 전제로 삼으면(규칙 2가 그렇게 쓰라고
한다), 그 부재를 **창에서 저작으로 뒷받침한다.** 사고를 서술하는 행만 있는 창에서는 그
전제가 언제나 거짓이고, 갈림길은 기본값을 낼 수 없다. 이 줄은 반증의 부재를 말해야지 사실을
주장해서는 안 된다 — 사실을 주장하면 규칙 3이 경고한 「열쇠가 통하지 않는 자물쇠」가 된다.

이것으로 A26의 포화 판정은 G1에 대해 철회한다. A27이 좁힌 대로, 정상화하는 줄을 써 보지
않은 채 내린 판정이었다.
