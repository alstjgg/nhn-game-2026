...
- 메커니즘 검증 국면을 종료한다. C-STRUCT는 **완전 종료**, C-BLOCK에 남아 있던 추가 검증 대부분도 **생략**
- 남은 기간(마감 ~08-10)에 완벽한 게임이 아니라 **작동하는 게임**을 만들기 위해서다. 다음 국면은 구현이 아니라 **명세**: 전체 아키텍처를 확정한 뒤에만 개발을 시작한다.
- 분담은 **윤석 = LLM Infrastructure / Call Inventory**, **민서 = Scenario**.
...
1. **C-STRUCT 완전 종료.** 양쪽 측정 프로그램(#94 · #95)이 독립적으로 같은 결론에
   ...
   않는다. **UI 연출로도 다시 다루지 않는다** (07-31 확정). *(추가: PR #94는
   "pause + 재개 조건"으로, #95 REPORT는 "closed · UI flavor로는 출하"로 적혀
   있다 — 문서 통합 패스에서 "closed · 완전 제거"로 통일할 것.)*
...
3. **명세-우선.** 절대 구현·개발·작업부터 시작하지 않는다. 전체 아키텍처를 확정한
   뒤에 개발을 시작한다. **아키텍처 확정 데드라인: ~08-02 (이번 주).**
...
- 검증된 매커니즘 기반으로 **시나리오 축소 + 재생성**.
  - *(갱신 07-31: 재생성은 「재앙 시나리오 집필 브리프」로 여러 LLM 세션(모델별·
    ...
    장소 5~8 · 숨겨진 진실 3~5. 브리프 문서는 레포에 두지 않고 생성 세션이 보유 —
    최종 시나리오 선정 시 초안과 함께 planning/에 아카이브 권장, "생성 지시서 →
    병렬 초안 → 선정" 과정이 AI 활용 문서(대회 제출물)의 재료가 된다.)*
- 시나리오에 귀속되는 것:
  - 게이트
  - 게이트별 스탠스
  - NPC 목록
...
- **우선순위 리스트 완전 제거 (07-31, 민서 결정):** C-STRUCT는 UI 연출로도 다시
  다루지 않는다 (결정 사항 1). 최소 엔진의 actuator whitelist에서 priority-reorder
  액추에이터 제외, UI 작업량에서도 제외.
