...
Coverage: read in full — `planning/scenario-model.md`, `planning/scenario-writer-brief.md`,
`planning/scenario-writer-brief-freetopic.md`, both free-topic drafts
(`planning/dday-scenario/drafts/자유주제-멈춘회전문.md`, `자유주제-전구간정상.md`),
`planning/dday-scenario/paper-check-전구간정상.md`, and the `테러리스트의전화-우는다리.md`
draft diff (5a3c388..origin/main). The two briefs are near-identical; shared rules are cited
to the non-free brief and the free-only differences to the free brief (one source per atom).
Not mined here: the `data/scenario/전구간정상/` and `data/scenario/멈춘회전문/` compiled
packs (JSON — same content as drafts per the paper-check), the g*-*/hf* PRD files under
`planning/prds/`, and `planning/dday-mechanism/` run/suite files (probe territory, other slices).

### S2-071 — Authoring order inverted: graph first, timeline derived last
- source: planning/scenario-model.md §4
- date: 2026-08-09
- lanes: 4
- event: The scenario decisions record fixed a graph-first writing order — endings → routes → the gate each route turns on → what the agent must know → what experience yields it → which run affords it → the timeline row — with the timeline written last and "entirely derived". This inverts the existing order, where the timeline was written second and gates fitted onto it seventh.
- tension: The team diagnosed that the old order made evidence land "wherever the story wanted it rather than where the routes need it," and reversed the pipeline to fix it.
- quote: "The timeline is the **last** artifact written and it is entirely derived. Every row is a row some route needs, a row that makes a needed row legible, or cut."
- links: S2-059
- flags: reversal, decision

### S2-072 — 기질 조건절 removed: the graph produces failure structurally instead
- source: planning/scenario-model.md §4.1
- date: 2026-08-09
- lanes: 4
- event: The decisions record removed the 기질 조건절 (temperament conditional clauses), which "existed to manufacture failure in a scenario that had no other way to produce it." It takes `key_conditions`, `key_examples` and the E-K1/E-K2 lint rules with it; the graph now produces failure because the default edge is the fail edge. The schema change is scoped as a relaxation so existing packs stay valid.
- tension: A whole locking mechanism was deleted once the graph could produce failure by shape — "the agent reasoning its way to an obvious key is a better thing for the player to read than a lock opening."
- quote: "The graph now produces failure structurally: the default edge is the fail edge, and nothing in the agent's knowledge supports another. No lock is needed."
- links: S2-071
- flags: reversal, boundary

### S2-073 — The 0.81 traced to a single line of scene prose
- source: planning/scenario-model.md §7
- date: 2026-08-09
- lanes: 4
- event: The record located the cause of the measured 0.81 run-1 determinism: G1's scene prose supplies the interpretation that turns 오세라's harmless complaint ("여기 바람이 안 옵니다") into evidence, before the agent decides — so the agent deviates without any handed sentence. The fix is that a gate's scene states the situation, never the contradiction; the target is p = 1, not 0.81.
- tension: A near-miss determinism number that read like model misbehaviour was re-attributed to one authored sentence, and PR #220's `excerpt` fix was reframed as having treated a symptom.
- quote: "**That single sentence is where the 0.81 comes from**, and it is why hiding timeline rows from the agent's window (PR #220's `excerpt`) was treating a symptom."
- links: S2-070
- flags: measurement, reversal

### S2-074 — Fail node redefined: agency ends, the clock keeps running
- source: planning/scenario-model.md §5
- date: 2026-08-09
- lanes: 4
- event: A fail node was defined not as a run terminator but as the point where the agent's agency ends while the clock keeps running: Call 1 stops (no question, no stance, no utterance), Call 2 and Call 3 keep firing so the disaster narrates and reports keep arriving. The consequence tail was named the primary source of route knowledge, making run 1 productive without the agent doing anything competent.
- tension: The design turned the failure state into the game's main teaching surface — "the agent's voice stops, the world keeps moving, and pages keep arriving" — verified against engine behaviour rather than added as new machinery.
- quote: "run 1 is **short on agency but full length on narration** — the right shape for a judge's first sixty seconds."
- links: S2-072
- flags: decision, boundary

### S2-075 — Two gates at one clock silently delete one
- source: planning/scenario-model.md §9.2 (A)
- date: 2026-08-09
- lanes: 2, 4
- event: Probing the engine, the record found that `buildSchedule` assigns a gate to a clock slot by plain assignment, so two gates at the same clock (probed: G1 and G2 both at 21:30) return only the second, with no error from compiler, schema, or lint. It was flagged the most dangerous finding because a graph naturally wants sibling nodes at the same depth, and demands a unique clock per node plus a new lint rule.
- tension: A silent data-loss bug directly in the path the new graph format would exercise, invisible to every existing guard.
- quote: "the schedule returns **only G2**, with no error from the compiler, the schema, or lint. Nothing anywhere checks it."
- links: S2-074
- flags: failure, measurement

### S2-076 — Two graph notations in the datapack; only one is wired
- source: planning/scenario-model.md §9.3
- date: 2026-08-09
- lanes: 2, 4
- event: The record found `edge_predicates` is "dead wiring" — its vocabulary is real and compiled, and `submitStance` returns the result as `nextNode`, but nothing in `src/` consumes it; only tests read it. Routing is done entirely by `availability` on the target gate, so the datapack carries two graph notations and only one is live. The graph format should target `availability`.
- tension: Discovering a fully-built, compiled routing mechanism that the shipping client never reads — leaving whether to adopt or delete it a deliberate decision rather than an inherited one.
- quote: "So the datapack carries **two** graph notations and only one is wired."
- links: S2-075
- flags: contradiction, measurement

### S2-077 — Minimum deaths is zero; the current pack is structurally barred from it
- source: planning/scenario-model.md §2
- date: 2026-08-09
- lanes: 4
- event: The record decided a run exists in which nobody dies and the player can reach it, with the price of a clean run paid "in something other than lives — prosecution, closure, a record that follows someone." It then derived that no single gate may resolve two scored units in opposite directions, and noted the current pack fails this: `doors_opened` empties the tunnel but kills 오세라 while `driven_out` saves 오세라 and leaves 41 in the tunnel.
- tension: A design rule ("however hard you try, someone dies" is a novel's ending, not a trainer's) that the existing shipped scenario provably violates, making a clean run impossible whatever the numbers say.
- quote: "there must exist at least one complete route that is simultaneously optimal for every scored unit."
- links: S2-074
- flags: decision, contradiction

### S2-078 — Gates ask what to believe, never what to command
- source: planning/scenario-writer-brief.md §4-15
- date: 2026-08 (best effort)
- lanes: 4
- event: The writer brief states its costliest measured rule: a gate must ask what the agent will believe and therefore ask next, not what it will command. After measuring twelve gates across two packs, every surviving gate's non-default action was an asking/lookup act (recount, check the roster, search the wall) and every dead gate's was a command (open the emergency door, move two hundred people).
- tension: A whole class of gate design was ruled out by measurement — commands sit outside the agent's 청취·조회·요청 authority and pay their cost immediately while the benefit needs a key, so the agent retreats to the cheap option.
- quote: "살아남은 게이트의 비기본 행동은 전부 **묻거나 조회하는 일**이었다 … 죽은 게이트의 비기본 행동은 전부 **명령**이었다"
- links: S2-060
- flags: measurement, boundary

### S2-079 — Measured: a stance label must not name the truth it unlocks
- source: planning/scenario-writer-brief.md §5-11
- date: 2026-08 (best effort)
- lanes: 4
- event: The brief records as "가장 비싸게 배운 것" that a non-default label may state the premise a key licenses but must not name the truth's content — because the agent then learns the truth just by reading the stance set, and an empty handoff opens the door. Measured: gates that named the object by name saw no-intervention baselines fall to 70% and 10%, while gates that stated only the interpretation held at 100%.
- tension: The line between a premise and its evidence was drawn by measurement — naming the chained door in the label ("사슬 때문이라고 보고") makes the handed key redundant.
- quote: "사물을 이름으로 부른 게이트는 무개입 baseline이 70%·10%까지 떨어졌고, 해석만 말한 게이트는 100%였다."
- links: S2-078
- flags: measurement, boundary

### S2-080 — Measured: preparation-only stances cost nothing and always win
- source: planning/scenario-writer-brief.md §5-14
- date: 2026-08 (best effort)
- lanes: 4
- event: The brief records that every stance must pay something; a stance that merely prepares ("보낸다 · 묻는다 · 자리를 잡는다") has cost zero and a zero-cost option always wins because no warning has anywhere to land. A stance must instead stake itself on the act ("설비 쪽으로 보내는 것이 아니라 지금 돌리게 하고"). Measured: a gate whose three non-default stances were all preparation held a 0% baseline even after three label rewrites and added world physics; a gate where all four options paid a cost was 100%.
- tension: A gate could be rendered dead by the reversibility of its options alone, independent of prose or key quality — verified by the 0% versus 100% split.
- quote: "비기본 셋이 전부 준비였던 게이트는 라벨을 세 번 고치고 세계에 물리를 넣어도 무개입 baseline이 0%였다. 네 선택지가 전부 값을 치르는 게이트는 100%였다."
- links: S2-079
- flags: measurement, boundary

### S2-081 — The single-window rule named the brief's most important constraint
- source: planning/scenario-writer-brief.md §1.1
- date: 2026-08 (best effort)
- lanes: 4
- event: The brief's stated most-important rule is that the only channel onto the situation is the one agent — its transmitted records and its written report — so every sentence the writer produces must be something the agent did, said, saw, heard, or thought, with no overhead camera anywhere. It is framed as world physics, not style: if the player ever knows more than the agent, the game flips from "find the truth and pass it" into "how do I convey what I already know."
- tension: The membrane constraint (structured input, no player omniscience) restated as an authoring physics that, if broken, collapses the core loop.
- quote: "플레이어가 요원보다 많이 알게 되는 순간, 게임은 「진실을 알아내서 요원에게 전한다」에서 「내가 이미 아는 것을 요원에게 어떻게 전달하나」로 바뀐다."
- links: S2-068, S2-082
- flags: boundary

### S2-082 — Free-topic brief hands the AI writer the whole disaster; tunnel fire is fenced off
- source: planning/scenario-writer-brief-freetopic.md §2
- date: 2026-08 (best effort)
- lanes: 4
- event: The free-topic variant of the brief tells the AI writer to invent the disaster, city, on-duty staff and hidden truth itself — only the physics is fixed (a report-triggered disaster, a hard-to-evacuate crowded place, three-digit deaths, a reachable zero-death ending, all times before midnight). It explicitly bans tunnel fire because another assignment already claimed it.
- tension: The team handed an AI writer open authorship of an entire scenario world while carving out one topic to prevent collision with a sibling assignment — a boundary drawn between two AI writing runs.
- quote: "**터널 화재는 쓸 수 없다.** 다른 과제가 이미 가져갔다. 그 밖에는 제한이 없다."
- links: S2-083
- flags: boundary, decision

### S2-083 — AI writer produced a full graph-first scenario on air-supported-dome physics
- source: planning/dday-scenario/drafts/자유주제-멈춘회전문.md §1–§9
- date: 2026-08-09
- lanes: 4
- event: From the free-topic brief the AI writer delivered a complete graph-first datapack draft ("멈춘회전문"): a snow-loaded air-pressure dome where 736 people exit through a two-at-a-time revolving door, three characters, three gates, two zero-death routes (restore fan #2 via the tripped breaker, or open the chained north door), a 16-row timeline, and a full aggregation table — and it passed its own §9 self-check.
- tension: An AI-authored scenario built the disaster's physics into the puzzle (opening a large aperture drops the air-held roof), meeting the brief's structural demands without human authoring.
- quote: "지금 사람이 걸어 다닐 수 있는 것은 안쪽 압력이 지붕을 들고 있기 때문이고, 큰 개구부를 열면 그 압력이 빠져 막이 내려온다"
- links: S2-082
- flags: milestone

### S2-084 — Closing seven blockers opened three new defects, all the same disease
- source: planning/dday-scenario/paper-check-전구간정상.md ①, ②
- date: 2026-08-09
- lanes: 4
- event: The second paper-check of the 전구간정상 pack confirmed all seven round-1 blockers closed, but found the fixes created three new hardening-layer defects — all the same disease: a bucket's flag mismatched what the stance actually did in the world, or the timeline did not branch to follow it. N1: the P14 fix gave 압박 `manifest_checked` while the flag `logs_opened` sat on 대조, which does not open the logs. N2: the P3 fix introduced `pallet_named` but the 00:12+ row did not follow the branch.
- tension: Repairing one layer of an AI-authored pack broke a deeper layer in the same pass, with the paper-check tracing each new bug to the exact fix that caused it.
- quote: "닫는 과정에서 하드닝 층에 새 결함 셋이 생겼다. 셋 다 같은 병이다 — 버킷의 flag가 그 stance가 세계에서 실제로 한 일과 어긋나거나, 고정 타임라인이 그 flag를 따라 갈리지 않는다."
- links: S2-083
- flags: failure, reversal

### S2-085 — 우는다리 hardened: numeric-vs-string tallies split, one flag left deliberately non-branching
- source: planning/dday-scenario/drafts/테러리스트의전화-우는다리.md §점수 (diff 5a3c388..origin/main)
- date: 2026-08 (best effort)
- lanes: 4
- event: The 우는다리 draft's score section gained an aggregation-rules block and rationale: only numeric tally values sum into the death/injury header, so string outcomes (verdict, detention hours, injured count) stay separate ("구금 6시간이 사망 6명에 더해지는 일은 없어야 한다"), and a new `logs_requested` flag was added but deliberately reads nowhere — the shredding receipt is not a copy of the ledger, so it splits no unit.
- tension: A scenario edit that both fixes a category error (mixing detention-hours into a death count) and knowingly leaves a flag inert, recording why rather than deleting it.
- quote: "`logs_requested`(파쇄 신청서 품목란 사본)는 장부를 가르지 않는다 … 팩 안에 이 flag를 읽는 분기도 없다."
- links: S2-084
- flags: boundary, decision
