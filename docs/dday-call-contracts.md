...
| `FIXED_NPC_ACTION` | 이번 비트의 고정 NPC 행동 | 서술 대상이 아니라 **모순 방지용 제약**. 이미 일어난 것으로 취급한다 |
| `SCENE_SYMPTOMS` | 엔진 delta journal이 증상 문장으로 렌더한 것 | 상태 변화의 유일한 통로 (규칙 5) |
| `PRESENT_NPCS` | `{id, name}` 목록 | `npc_lines`의 화자는 여기서만 나온다 |
...
| 4 | `SCENE_SYMPTOMS` 렌더러 계약 | 미결 — [엔진 최소 명세 요청서](./dday-engine-minimal-request.md) §6-2로 이관 | 엔진 |
| 5 | 콜 실패 시 게임 거동 | **미정의** — [엔진 최소 명세 요청서](./dday-engine-minimal-request.md) §6-3으로 이관. 프록시의 fallback 메타데이터 계약도 함께 | L + 엔진 |
