...
| **Run-loop Manager** — the multi-run shell | run record · block-store state at run end | **meta-state** (its artifact: run counter · report archive · carried blocks · exposure clock) · exposure depth → State Engine · archive + counter → View | `src/runloop/`, isomorphic ([plan-engine-build](./plan-engine-build.md) e8); meta-state in `sessionStorage` (physical §1.1, 08-03 — headless disk mirror is the manager's own call) | [plan-pipeline §3](./plan-pipeline.md) · [contract-run-artifacts](./contract-run-artifacts.md) · [physical §1.1](./spec-physical-architecture.md) |
