...
| Nothing sets `VITE_PROXY_BASE_URL`; `deploy.yml` must not change | 윤석, self | **open** — the URL now exists (`aws cloudformation describe-stacks --stack-name nhn-game-proxy`, output `CallEndpoint`; not written down here, since the endpoint is unauthenticated), and nothing calls `createTransport` yet either, so this binds with the client↔live-driver wiring rather than on its own | [`contract-calls.md`](./contract-calls.md) §11 |
