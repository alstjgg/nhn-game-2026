...
| **e4** | `src/engine/` feed + round assembler: `feed()` with minted ids per decision 6 · `EXPERIENCED` assembly · `inner_note` isolation | e0·e3 | vitest: contract §8 criteria 1·5·6 as written |
| **e5** | `src/composer/`: the three builders · block-id resolution + canonical sort · proxy-owned slots never emitted | e1·e3 | vitest: contract §8 criteria 4·7 (criterion 7 runs the payload through the proxy's own validators, offline) |
...
| **e10** | acceptance: contract §8's seven criteria as one suite + `npm run check`/`build` green + the probe's 44 and the proxy's 39 still passing | all | **full suite green — the only whole-suite gate** |
