...
| E7 | **증상 커버리지** — 액추에이터(버킷 델타 + 타임라인 이벤트 `effects`)가 만들 수 있는 모든 (변수, 방향)에 `min: 1`까지 내려오는 항목, 플래그 set/unset에는 대응 문장. 없으면 런타임 hard error가 그 델타를 밟는 경로에서만 터진다. 델타·효과가 비어 있으면 공허하게 통과(하드닝 전) | 엔진 명세 §6-2 (개정 요청 수락, 08-01) · `effects` 필드는 §3 v0.2에서 신설 |
