...
| E7 | **증상 커버리지** — 버킷 델타가 만들 수 있는 모든 (변수, 방향)에 `min: 1`까지 내려오는 항목. 없으면 런타임 hard error가 그 델타를 밟는 경로에서만 터진다. 델타가 비어 있으면 공허하게 통과(하드닝 전) | 엔진 명세 §6-2 (개정 요청 수락, 08-01). 타임라인 이벤트 효과는 §3에 필드가 생길 때 소스에 합류 |
