...
demos/dday-simulation/poc-terror/PAPER-TEST.md 를 읽고, 그 지침을 **문자 그대로**
따라 페이퍼 테스트를 실행하라. 세계 데이터는
demos/dday-simulation/poc-terror/slice-terror.json.
...
- 절대 규칙(§0) 요약: 판단·보고서는 반드시 `sim-field-haiku-*` 전용 정의
  ...
  한다. slice 수치 변경 금지.
- 산출물은 demos/dday-simulation/poc-terror/runs/ 와 RESULTS.md 에 남긴다.
